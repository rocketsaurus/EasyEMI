...
class mathPopup(QWidget):
  ...
  def initUI(self):
    ...
    okayBtn.clicked.connect(self.plot_result)
    cancelBtn = QPushButton('Cancel')
    cancelBtn.setMaximumWidth(100)
    cancelBtn.clicked.connect(self.hide)
    ...
    grid.addWidget(self.math_combo, 1, 1, 1, 1)
  ...
  def initCombos(self):
    ...
    operations = ['-', '+', '*', '/']
    self.math_combo.addItems(operations)
  ...
  def plot_result(self):
    ...
    df1 = pd.merge_asof(df1.iloc[:,0:2], df2.iloc[:,1], left_index=True, right_index=True, direction='nearest', tolerance=1)

    if self.math_combo.currentText() == '-':
      df1['Result'] = df1.iloc[:, 1] - df1.iloc[:, 2]
    elif self.math_combo.currentText() == '+':
      df1['Result'] = df1.iloc[:, 1] + df1.iloc[:, 2]
    elif self.math_combo.currentText() == '*':
      df1['Result'] = df1.iloc[:, 1] * df1.iloc[:, 2]
    elif self.math_combo.currentText() == '/':
    ...
    if self.name.text():
      result_label = self.name.text()
      filename = result_label + '.csv'
      df1[['Frequency (MHz)', 'Result']].to_csv(self.parent.workspace / filename, index=False)
    else:
      result_label = self.x_combo.currentText() + self.math_combo.currentText() + self.y_combo.currentText()

    self.parent.main_plot.plot(df1.iloc[:, 0], df1['Result'], label=result_label)
...
class EasyEmi(QMainWindow):
  ...
  def __init__(self):
    ...
    # Status Bar
    self.statusBar().showMessage('Version 2.5')
  ...
  def initToolbar(self):
    ...
    mathAct = QAction('Math', self)
    mathAct.setStatusTip('Select two traces and perform mathmatical operation +,-,*,/')
    toolbar.addAction(mathAct)
    mathAct.triggered.connect(self.show_math_popup)
  ...
  def connect_to_sa(self):
    # Attempt connection with spectrum analyzer.
  ...
  @pyqtSlot()
  def show_math_popup(self):
    self.math_popup = mathPopup(self)
    self.math_popup.setGeometry(400, 100, 250, 150)
    self.math_popup.show()
  ...
  def load_file(self, filepath, trace):
    ...
    if fp.lower().endswith('.txt'):
      self.data[trace] = pd.read_csv(filepath, sep='\t', encoding='latin-1')
      self.data[trace] = self.data[trace].loc[:, ~self.data[trace].columns.str.contains('^Unnamed')]
    else:
      with open(filepath, 'r', encoding='latin-1') as csvfile:
        ...
        if its_data and len(firstline) >= 2:
          ...
          try:
            ...
            print('File format may not be supported.  Please contact jchinn@google.com with file.')
  ...
  def remove_plot(self, trace):
    ...
    if its_val[0] in legend and its_val[1] in legend: 
      ...
      if not self._check_legend_empty():
        ...
        self.redraw_plots()
    elif its_val[2] in legend and its_val[3] in legend:
      self.vert_plot.lines.pop(vlegend.index(its_val[2]))
      self.horiz_plot.lines.pop(hlegend.index(its_val[3]))
      self.main_plot.lines.pop(legend.index(its_val[2]))
      legend = self.main_plot.get_legend_handles_labels()[1]
      self.main_plot.lines.pop(legend.index(its_val[3]))
      if not self._check_legend_empty():
        self.redraw_legend([0, 1, 2])
        self.redraw_plots()
  ...
  def dbl_clk_tree(self, index):
    ...
    if filetype == 'csv File' or filetype == 'DAT File' or 'txt File':
      ...
      legend = self.main_plot.get_legend_handles_labels()[1]
      its_val = [trace + ' - Vertical', trace + ' - Horizontal',
                  trace + ' - Phase 1', trace + ' - Phase 2', trace]

      #if trace not in legend and its_val[0] not in legend and its_val[1] not in legend:
      if not any(elem in its_val for elem in legend):
  ...
  def plot_trace(self, trace_name):
    ...
    if 'Position' in column_headers:
      ...
      # if CE, split on Phase 1/2
      if self.data[label_v].empty and self.data[label_h].empty:
        self.data.pop(label_v)
        self.data.pop(label_h)
        label_v = trace_name + ' - Phase 1'
        label_h = trace_name + ' - Phase 2'

        if label_v not in self.data:
          self.data[label_v] = self.data[trace_name].loc[self.data[trace_name]['Position'] == 'Phase 1']
        if label_h not in self.data:
          self.data[label_h] = self.data[trace_name].loc[self.data[trace_name]['Position'] == 'Phase 2']
